...
射擊遊戲
1. 基礎打擊感音
受擊閃白（Hit Flash）：基礎受擊視覺反饋。  
音效搭配：加入開槍與命中音效。  
2. 槍械與射擊動態效果
槍口開火特效：加入槍口火星粒子與換彈時的微弱煙霧效果。  
後座力抖動（Recoil）：微調武器抖動以模擬射擊後座力。  
換彈與切槍：製作拾取武器、換彈匣與切換槍械的過渡動畫。  
3. 受擊與環境反饋（Impact Feedback）
受擊粒子：怪物挨打時噴血，地面或牆面受擊時噴射火星粒子。  
受擊形變（Squish & Stretch）：受擊單位產生輕微的擠壓與拉伸形變。  
傷害數字（Floating Text）：經典的受擊跳數字效果。  
子彈擊退（Knockback）：根據子彈衝力給予目標物理擊退效果。  
螢幕震動（Screen Shake）：相較於開槍震動，作者選擇在玩家受擊時觸發螢幕震動，以提升警示感且避免過度晃動造成暈眩。  
4. 道具與特殊武器實現
投擲物：投擲手雷並產生爆炸效果。  
噴火槍（Flame Thrower）：使用粒子系統（Particle System）實現火焰噴射。  
雷射槍（Laser Gun）：透過 Shader 繪製直線與動態光效。  

<hr>

### 參考項目
1. Godot 4《3D 地牢爬行者》https://youtu.be/fkSegL1vLrk?si=84eb4FkC5pDiZmsz 
2. 

<hr>

###  游戏规则改造与迭代指南（三坐标八象限博弈系改造法）
中国象棋
|X ：信息（完全 vs 不完全）|Y ：随机性（确定性 vs 随机性）|Z ：博弈结构（动态 vs 静态 / 竞技性）|
|--|--|--|
|[玩家仅能看到棋子视野内的区域]|[（引入微量随机）：每局提供三次掷骰机会，奇数解除兵/卒移动方向限制，偶数解除马/象蹩脚限制]|[（引入静态博弈）：每隔 10 回合触发一次“同时行动”，双方将策略写在纸上/系统判定同时移动（类似猜拳博弈）]|
...
[偽3d效果>2.5D||互動式動態霧氣||派對屬性||AtlasTexture 拼圖]
搜打撒:4邦布/就地生成/探測器
...
2. 彩虹字,浮動,互動流體模擬,卡反絕區零火鍋,3D 掃雷, 瘋狂鴿子3d /滾動的天空/ 號角保齡球/ 閃避球/ 滑雪/ 美夢zzz/ TNT弹道轨迹
11. 宝箱|门|NPC|子弹|抓钩勾索,傳送門槍,coin〉升級,爬墻,動作設計：技能普攻/強化/大招,無縫第三人稱視角傳送門+robot核心就是key
...
kimodo
根據角色設計{闲置、行走、奔跑、跳跃,
 **攻擊,強化攻擊,大招,閃避}16幀動作幀prompt**  ,不需要文字+Frame number  我的意思是每個動作都是16幀動作幀,設計 prompt
 场景为@f907e79c-982f-486f-99c6-341c17ed3148（里面有一些科技木桩），@bf01af55-a638-4ab6-ba34-9759b9412fb8为一把重剑，纯侧面远景：@10b6842a-6dd9-4a23-b698-1f0578c79989闭着眼在@f907e79c-982f-486f-99c6-341c17ed3148场景里的科技木桩的对立面（@bf01af55-a638-4ab6-ba34-9759b9412fb8在角色身前竖直立在地上），中景：@10b6842a-6dd9-4a23-b698-1f0578c79989一只手倒持@bf01af55-a638-4ab6-ba34-9759b9412fb8手指扣紧刀柄另一只手准备拔剑式（剑身的噪波粒子微光反应越来越激烈），面部特写：睁眼，伴随粒子发光特效，镜头威震，伴随时不时迸发的粒子能量嗡嗡声，再切@10b6842a-6dd9-4a23-b698-1f0578c79989手部近景一镜到底拉远至角色全身：角色快速拔剑向前冲锋（阿尔法突袭）化作一道粒子流光并在原地诞生一个分身残影，同时带起一阵风引起镜头震动，同时镜头快速切至全部科技木桩的背后拍向@10b6842a-6dd9-4a23-b698-1f0578c79989的视角：同时角色从远处以流光突刺阵冲击每一个科技木桩然后闪现到镜头最前面的，同时背后的科技木桩按从远到近的流光挨个破碎成数个碎块，伴随绿色的粒子流光迸发效果，同时有清脆的爆破声，接着@10b6842a-6dd9-4a23-b698-1f0578c79989在原地把@bf01af55-a638-4ab6-ba34-9759b9412fb8斜向反握斜向放在角色背后的地下（剑尖置地），另一只手叉腰，松了口气，视频无字幕，无背景音乐，角色持重剑要挥动要有重量感，场景全程有风动气场，吹动角色衣服和头发，角色动作连贯，
...
格挡框架解析
应对与特效：玩家响应后，主体触发描边改色、向前突进及武器前十字准备特效，进入格挡状态。 
镜头与后处理：相机快速下沉、画幅倾斜，屏幕触发振动与镜像模糊。 
碰撞卡帧：碰撞发生时播放音效与特效（速率呈“快入中频缓出”曲线）；双方角色动作播放速度修正为 0（卡帧）。 
收尾重置：卡帧结束后，双方播放收尾动画，相机、描边（Outline）及周边受重力下降的粒子特效缓出重置。 
“快进缓出”速率曲线：
前腰急促：前腰几乎没有，伤害判定帧前非常急促，且前几段攻击可被任意打断，用于配合玩家主体完全定格的卡帧机制。 
后腰放缓：判定帧结束后动作放缓，展示大量刺激动作（传递角色个性样本）。 
动作设计原则：
大开大合与差异化：避免单一抬手斩击，需穿插躯体切换、转身下潜、纯特效或二号武器。招式逐级上升（如鲨鱼妹：1A 垂直劈砍 → 2A 旋转反身双斩 → 3A 回身斩+踢枪）。 
三、 相机（Camera）与视觉后处理
FOV 缩放（拉高 vs. Zoom）：拉高 FOV：视野广、边缘畸变，用于表现广域范围攻击。 Zoom 缩放：画面放大传递压迫感（如连携技）；增强感官冲击并减少背景信息量，常用于前进下砸（如 5A、大招多段）。 
五、 动作叙事与体验曲线设计
体验曲线（非正负归零陷阱）：拒绝“上升-下降-上升-下降”的陷阱曲线，采用不断上升强化的体验曲线，并在结尾设置猛烈的锚定点（高峰体验）。 以 5A 连段为例：1A 快斩 → 2A 翻身双斩 → 3A 穿梭冲刺 → 4A 升龙踢 → 5A 砸地重击（强冲击锚定）。 
大招镜头编排：第一镜头对脸，强调角色表情与眼睛（如眼神收束、变红、帽子落下等过场）。 环境改色、强对比闪光、速度线、色散等后处理全开。 招式保持“快斩 → 回旋重斩 → 上挑 → 独立镜头蓄力最终击”的递进结构，在保持效果语言一致的同时完成强度匹配。 	
</details>
...
**從「肌肉記憶」到「敘事與審美」：作品的真正價值不在於點擊滑鼠有多快或熬夜調材質的「苦勞」，而在於創作者的審美、個人閱歷、敘事邏輯與個人認知。**
