...
5. camera delay
6. 创意的定义与三大前提 **玩家能理解, 新的东西,新的体验**|设定清晰体验目标|对默认规则动手|行为系统的有机融合|打破知识断层与多元积累
<hr>
...
<details><summary> ###  游戏规则改造与迭代指南（三坐标八象限博弈系改造法）</summary>
X ：信息（完全 vs 不完全）[中国象棋（引入不完全信息）：加入“战争迷雾”机制，玩家仅能看到棋子视野内的区域（如拉大兵/卒的视野，使车马炮依赖兵卒探路）。|梯蛇棋（引入不完全信息）：投出点数（如 6 点）可选择不移动，改为抽取一张对敌/对己施加效果的“隐蔽锦囊牌”（如使对方传送无效）。]
Y ：随机性（确定性 vs 随机性）[中国象棋（引入微量随机）：每局提供三次掷骰机会，奇数解除兵/卒移动方向限制，偶数解除马/象蹩脚限制，创造“神之一手”。|梯蛇棋（降低过高随机）：投出 6 点可换取固定点数的遥控骰子（最多持有 5 个），让玩家能主动控制步数。]
Z ：博弈结构（动态 vs 静态 / 竞技性）[中国象棋（引入静态博弈）：每隔 10 回合触发一次“同时行动”，双方将策略写在纸上/系统判定同时移动（类似猜拳博弈）。|梯蛇棋（提高竞争性/飞行棋化）：增加为每人 4 个棋子（需投出 6 点启动），落到他人棋子格可将其撞回起点，棋子可叠放移动。]
控制论：正反馈与负反馈的应用
</details>
