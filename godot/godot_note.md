...
5. 创意的定义与三大前提
AI 虽然能够深度参与游戏开发的各个环节，但独立游戏最终比拼的是“独一无二的体验”。一个合格的游戏创意必须同时满足： 
玩家能够理解：能与玩家形成有效沟通。 
提供新的东西：拒绝无意义的换皮。 
产出新的体验：避免枯燥重复的机械任务（如不断复读的通马桶任务）
打造核心创意的四大方法
设定清晰体验目标：设计创意前需明确“让玩家在什么情况下，通过什么行为，感受到什么”。不要仅定位于“做个酷炫的赛博朋克游戏”，而是要聚焦于玩家的具体行为与情感反馈。 
对默认规则动手：挑战游戏常识（如控制、死亡、升级、背包、地图等规则）。修改规则的目的是为了产生新选择与新体验（如走路产生声音影响隐蔽），而不是单纯增加限制。 
MD+ 1
行为系统的有机融合：创意的组合不是视觉元素的机械拼接（如克苏鲁+赛博朋克），而是将两个行为系统接在一起（如将地图探索融入道具合成、将背包整理变成战场布阵）。各子系统间要形成闭环与互相驱动的循环关系。 
打破知识断层与多元积累：创意的原料来自于对现实世界与跨学科知识的观察（如经济学杠杆、生物学合作、社会资源调动等），将这些规律“翻译”为游戏规则。 
...
核心特征：输出完全可控，对抗本质在于心理预判与博弈。 

X 坐标：信息（完全 vs 不完全）
中国象棋（引入不完全信息）：加入“战争迷雾”机制，玩家仅能看到棋子视野内的区域（如拉大兵/卒的视野，使车马炮依赖兵卒探路）。
梯蛇棋（引入不完全信息）：投出点数（如 6 点）可选择不移动，改为抽取一张对敌/对己施加效果的“隐蔽锦囊牌”（如使对方传送无效）。

Y 坐标：随机性（确定性 vs 随机性）
中国象棋（引入微量随机）：每局提供三次掷骰机会，奇数解除兵/卒移动方向限制，偶数解除马/象蹩脚限制，创造“神之一手”。
梯蛇棋（降低过高随机）：投出 6 点可换取固定点数的遥控骰子（最多持有 5 个），让玩家能主动控制步数。
