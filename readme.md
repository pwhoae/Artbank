[回到底部](#readme)<a id="readme_begin"></a><br>https://github.com/pwhoae/Artbank/blob/main/GPT%E7%94%9F%E6%88%90/readme.md

https://github.com/pwhoae/artbank_private
音頻:[shock]/ [cat_meme]/ [meme]/ [綜藝]
```
卖什么→谁来卖→卖给谁→怎么卖>钱怎么分→环境影响→行业怎么来的>怎么赚钱→凭什么赢→未来能走多远
```
遊戲democode learn/<a href="https://github.com/Kavex/GameDev-Resources">git(GameDev tag)</a>/<a href="https://itch.io/game-assets/tag-art">itch</a>/<a href="https://www.artstation.com/?sort_by=community&dimension=all">artstation</a>


### 美術
1. SVG网站：https://www.svgrepo.com/
2. 3d模型:https://www.blender-models.com/
3. https://poly.pizza/
4. https://kenney.nl/assets
5. https://polyhaven.com/
6. https://kaylousberg.itch.io/
7. sketchfab.com/cubebrush.co/cgmodel.com/free3d.co/
8. blender.org/download/demo-files/blendermarket.com
9. UML:https://plantuml.com/zh/
10. xmind(心智圖):https://xmind.com/zh-hant/
11. desmos(數學graph):https://www.desmos.com/?lang=zh-TW
12. lucidchart(flowchart):https://www.lucidchart.com/pages
13. flexible animatesheet player
14. AI放大+修复图片网站：https://bigjpg.com/
15. 抠图网站：https://www.koukoutu.com/removebgtool/all
16. 畫像素:https://www.piskelapp.com/
17. self-write去背:RemoveBG/index.html
18. https://yanstory.github.io/5000choyen/
19. 2d wall:https://wareya.github.io/webtyler/
20. tween:https://qaqelol.itch.io/tweens
21. [vercel.app][https://http.cat/]
22. 纹理网站：
23. 字体识别网站：
24. 字体网站：https://www.fonts.net.cn/
25. https://nieobie.itch.io/free-icons
26. 一键绑定模型骨骼：www.mixamo.com
27. 风格统一的素材：www.kenney.nl
28. ui图标：www.svgrepo.com
29. 音乐音效：pixabay.com<br>https://unity.com/cn/blog/games/level-up-your-code-with-game-programming-patterns
30. https://quaternius.itch.io/universal-animation-library/download/eyJpZCI6MzQwODAzNCwiZXhwaXJlcyI6MTc4MTUzMDQyN30%3d%2ea9%2bGjXyn%2bo0ydcDE%2fEREJdkicWY%3d
https://lightpdf.com/tw/
https://free3d.com/3d-models/animated?dd_referrer= 
https://www.mixamo.com/#/
https://kenney.nl/assets/category:3D 


### 音樂
n-bit:https://test-n-bit-audio-converter.vercel.app/<br>
suno:https://suno.com/create<br>
elevenlabs:https://elevenlabs.io/<br>
Strudel<br>

### 社交
|社交|links|
|------|---------|
|Youtube|Youtube懶人包<br>sponsorblock|
|BiliBili|黑名單:BiliBlock<br>封廣告:小电视空降助手|
|thread|影片下載:https://threadsdownloader.com/zh|
||https://ikigaitool.com/result/Vj9gQALBtx|
### 遊戲
|遊戲|links|
|------|---------|
|神魔之塔|主動技搜尋器:https://tinghan33704.github.io/tos-tools/#/tos-skill-filter<br>|
|絕區零|黎愔成就:https://zzz.liyin.space/achievement/0/0 <br> 面板:https://enka.network/?z|

[回到頂部](#readme_begin) 
<a id="readme"></a>
https://hkust-geco.gump.gg/album/day-2-november-15-sessions-5-8-ais-shss-seng<br>
# **🎨 Artbank & GameDev Resource Hub**

本儲存庫整理了遊戲開發、美術繪圖、音樂音效、圖像處理及日常實用工具的精選連結與備忘錄。

## **📌 目錄 (Table of Contents)**

* [🔗 相關專案與連結](#bookmark=id.6yupuzuwbe1j)  
* [💡 商業邏輯思考框架](#bookmark=id.cpxzwa2w6300)  
* [🎨 2D / 3D 美術與素材資源](#bookmark=id.oumkebv79rso)  
* [🛠️ 開發、圖表與圖像處理工具](#bookmark=id.t5ikmwwgb6wf)  
* [🎵 音樂與音效資源](#bookmark=id.n04ocyazw8b9)  
* [🎮 遊戲開發與專用工具](#bookmark=id.t6ba8grgj5c6)  
* [🌐 社交與瀏覽器擴充](#bookmark=id.2x4zncg5yvsd)  
* [📷 其他記錄](#bookmark=id.ydqxyzy00kjk)

## **🔗 相關專案與連結**

* 📄 [GPT 生成 README 歷史版本](https://github.com/pwhoae/Artbank/blob/main/GPT%E7%94%9F%E6%88%90/readme.md)  
* 🔒 [Artbank Private Repo](https://github.com/pwhoae/artbank_private)  
* 🎮 [GameDev Resources List (GitHub)](https://github.com/Kavex/GameDev-Resources)  
* 🎨 [Itch.io Game Assets (Art Tag)](https://itch.io/game-assets/tag-art)  
* 🖼️ [ArtStation Community Top](https://www.artstation.com/?sort_by=community&dimension=all)

## **💡 商業邏輯思考框架**

> **產品發想與商業化演進鏈條：**

> 賣什麼 → 誰來賣 → 賣給誰 → 怎麼賣 → 錢怎麼分 → 環境影響 → 行業怎麼來的 → 怎麼賺錢 → 憑什麼贏 → 未來能走多遠

## **🎨 2D / 3D 美術與素材資源**

### **🧊 3D 模型 / 動畫 / 骨骼綁定**

| 平台 / 工具 | 描述 / 連結 |
| :---- | :---- |
| **Mixamo** | AI / 一鍵模型骨骼自動綁定與動作庫：[mixamo.com](https://www.mixamo.com/) |
| **Kenney Assets** | 跨平台、風格統一的免費 2D/3D/音效素材：[kenney.nl](https://kenney.nl/assets) |
| **Poly Pizza** | 輕量級 Low-Poly 3D 模型庫：[poly.pizza](https://poly.pizza/) |
| **Poly Haven** | 高畫質 CC0 3D 模型、紋理與 HDRI：[polyhaven.com](https://polyhaven.com/) |
| **Blender Models** | Blender 社群模型庫：[blender-models.com](https://www.blender-models.com/) |
| **Blender Demo Files** | Blender 官方示範檔下載：[blender.org/download/demo-files](https://www.blender.org/download/demo-files/) |
| **Blender Market** | Blender 專業外掛與模型市場：[blendermarket.com](https://blendermarket.com) |
| **Kay Lousberg (Itch)** | 免費 Low-Poly 3D 資源庫：[kaylousberg.itch.io](https://kaylousberg.itch.io/) |
| **Quaternius Animation** | 通用動畫庫：[Itch Download](https://quaternius.itch.io/universal-animation-library/download/eyJpZCI6MzQwODAzNCwiZXhwaXJlcyI6MTc4MTUzMDQyN30%3d%2ea9%2bGjXyn%2bo0ydcDE%2fEREJdkicWY%3d) |
| **Free3D / Free3D Animated** | 免費 3D 與動畫模型：[free3d.com](https://free3d.com) | [Animated Models](https://free3d.com/3d-models/animated) |
| **Sketchfab** | 3D 模型預覽與交易平台：[sketchfab.com](https://sketchfab.com) |
| **Cubebrush** | CG 素材與工具市場：[cubebrush.co](https://cubebrush.co) |
| **CGModel** | 模型分享網：[cgmodel.com](https://cgmodel.com) |

### **🖼️ 2D / SVG / 圖標 / 字型 / 紋理**

* **SVG 圖標庫**：[SVG Repo](https://www.svgrepo.com/)  
* **UI 圖片與免費 Icon**：[Nieobie Free Icons (Itch)](https://nieobie.itch.io/free-icons)  
* **像素繪圖工具**：[Piskel](https://www.piskelapp.com/)  
* **2D 牆面 / 紋理無縫拼接**：[Webtyler](https://wareya.github.io/webtyler/)  
* **字體下載網站**：[字體天下](https://www.fonts.net.cn/)

## **🛠️ 開發、圖表與圖像處理工具**

### **📊 圖表與心智圖 (UML / Flowchart / Math)**

* **UML 繪圖 (PlantUML)**：[plantuml.com/zh/](https://plantuml.com/zh/)  
* **心智圖 (XMind)**：[xmind.com](https://xmind.com/zh-hant/)  
* **流程圖 (Lucidchart)**：[lucidchart.com](https://www.lucidchart.com/pages)  
* **數學函數繪圖 (Desmos)**：[desmos.com](https://www.desmos.com/?lang=zh-TW)

### **✂️ 圖像處理 / AI 放大 / 梗圖生成**

* **AI 圖片無損放大**：[BigJPG](https://bigjpg.com/)  
* **線上去背工具**：[摳摳圖 RemoveBG](https://www.koukoutu.com/removebgtool/all)  
* **本機自寫去背工具**：RemoveBG/index.html  
* **5000兆円 產生器**：[5000兆円 Generator](https://yanstory.github.io/5000choyen/)  
* **HTTP 狀態碼貓咪**：[http.cat](https://http.cat/)  
* **PDF 線上處理**：[LightPDF](https://lightpdf.com/tw/)

### **🎞️ 動畫與 Tween**

* **Tween 視覺化與資產**：[QAQELOL Tweens (Itch)](https://qaqelol.itch.io/tweens)  
* **Flexible Animatesheet Player** *(專案內組件)*

## **🎵 音樂與音效資源**

### **常用音效分類備忘**

* \[shock\] (震驚)  
* \[cat\_meme\] (貓咪迷因)  
* \[meme\] (梗圖/迷因)  
* \[綜藝\] (綜藝音效)

### **音訊工具與 AI 音樂**

* **N-Bit 音訊轉換器**：[N-Bit Audio Converter](https://test-n-bit-audio-converter.vercel.app/)  
* **Suno AI**（AI 音樂生成）：[suno.com/create](https://suno.com/create)  
* **ElevenLabs**（AI 語音生成/配音）：[elevenlabs.io](https://elevenlabs.io/)  
* **Strudel**（ Live Coding 音樂生成）：[strudel.cc](https://strudel.cc/)  
* **Pixabay Audio**（免費無版權音樂音效）：[pixabay.com](https://pixabay.com/)

## **🎮 遊戲開發與專用工具**

### **📖 設計模式與學習**

* **Game Programming Patterns (Unity Blog)**：[Level up your code with game programming patterns](https://unity.com/cn/blog/games/level-up-your-code-with-game-programming-patterns)

### **🎯 遊戲特定工具 (Game Helpers)**

* **神魔之塔 (Tower of Saviors)**：[主動技搜尋器](https://tinghan33704.github.io/tos-tools/#/tos-skill-filter)  
* **絕區零 (Zenless Zone Zero)**：  
  * [黎愔成就查詢系統](https://zzz.liyin.space/achievement/0/0)  
  * [Enka Network 角色面板查詢](https://enka.network/?z)

## **🌐 社交與瀏覽器擴充**

| 平台 | 建議擴充 / 工具 | 描述 / 連結 |
| :---- | :---- | :---- |
| **YouTube** | SponsorBlock / 懶人包 | 自動跳過影片贊助與片頭片尾 |
| **Bilibili** | BiliBlock / 小電視空降助手 | 避開黑名單內容與自動空降 |
| **Threads** | Threads Downloader | 影片下載工具：[threadsdownloader.com](https://threadsdownloader.com/zh) |
| **自我探索** | Ikigai 生涯規劃工具 | [Ikigai Tool Results](https://ikigaitool.com/result/Vj9gQALBtx) |

## **📷 其他記錄**

* 📸 [HKUST GECO Photo Album (Day 2 Nov 15\)](https://hkust-geco.gump.gg/album/day-2-november-15-sessions-5-8-ais-shss-seng)

[回到頁首](#bookmark=id.tlmsbyrkjwyp)


